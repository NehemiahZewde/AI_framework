...
from .nested_ml_tuning_calib_interpret import make_outer_inner_cv
...
def validate_nested_splits(
    ...
    strict_partition_check: bool = True,
    verbose: bool = True,
) -> Dict[str, Any]:
    """
    Validate a nested split plan produced by build_nested_cv_splits.

    Checks (outer):
      - correct number of outer folds: num_trials * n_outer_splits
      - indices within bounds, integer-like
      - no overlap between outer_train and outer_test
      - (optional) outer_test folds form a partition (each sample appears in test exactly num_trials times)

    Checks (inner) for each outer fold:
      - correct number of inner folds: n_inner_splits
      - inner indices are subsets of outer_train indices
      - no overlap between inner_train and inner_val
      - (optional) inner_val folds form a partition of outer_train (each outer-train sample appears in val exactly once)

    If groups provided:
      - group leakage checks for outer and inner: no group appears in both train and test/val.

    Returns a report dict with counts + basic label balance per fold.
    Raises AssertionError with a helpful message if something is wrong.
    """
    num_trials = cfg_cv["num_trials"]
    n_outer_splits = cfg_cv["n_outer_splits"]
    n_inner_splits = cfg_cv["n_inner_splits"]

    assert isinstance(nested_splits, list) and len(nested_splits) > 0, "nested_splits must be a non-empty list."

    expected_outer = num_trials * n_outer_splits
    assert len(nested_splits) == expected_outer, (
        f"Expected {expected_outer} outer folds (num_trials*n_outer_splits) "
        f"but got {len(nested_splits)}."
    )

    def _as_np_int(arr, name: str) -> np.ndarray:
        a = np.asarray(arr)
        assert a.ndim == 1, f"{name} must be 1D, got shape {a.shape}."
        assert np.issubdtype(a.dtype, np.integer), f"{name} must be integer dtype, got {a.dtype}."
        return a

    def _check_in_bounds(idx: np.ndarray, name: str):
        assert idx.size > 0, f"{name} is empty."
        assert idx.min() >= 0 and idx.max() < n_samples, (
            f"{name} has out-of-bounds indices. min={idx.min()}, max={idx.max()}, n_samples={n_samples}"
        )

    def _check_disjoint(a: np.ndarray, b: np.ndarray, name_a: str, name_b: str):
        inter = np.intersect1d(a, b)
        assert inter.size == 0, f"{name_a} and {name_b} overlap by {inter.size} indices (e.g. {inter[:10]})."

    def _counts_and_percents(idx: np.ndarray) -> Dict[str, Any]:
        vals = y[idx]
        c = Counter(vals.tolist())
        n = len(vals)
        p = {k: (v / n) * 100.0 for k, v in c.items()}
        return {"counts": dict(c), "percents": p, "n": n}
    ...
    for i, fold in enumerate(nested_splits):
        assert "trial" in fold and "outer_fold" in fold, f"Fold {i} missing 'trial' or 'outer_fold'."
        key = (fold["trial"], fold["outer_fold"])
        assert key not in seen_keys, f"Duplicate outer fold key {key} encountered."
        seen_keys.add(key)

        outer_train_idx = _as_np_int(fold["outer_train_idx"], f"fold[{i}].outer_train_idx")
        outer_test_idx  = _as_np_int(fold["outer_test_idx"],  f"fold[{i}].outer_test_idx")

        _check_in_bounds(outer_train_idx, f"fold[{i}].outer_train_idx")
        _check_in_bounds(outer_test_idx,  f"fold[{i}].outer_test_idx")
        _check_disjoint(outer_train_idx, outer_test_idx, "outer_train_idx", "outer_test_idx")

        # y_outer_* consistency (you said you ALWAYS track these)
        assert "y_outer_train" in fold, f"Fold {i} missing required 'y_outer_train'."
        assert "y_outer_test" in fold,  f"Fold {i} missing required 'y_outer_test'."

        y_outer_train = np.asarray(fold["y_outer_train"])
        y_outer_test = np.asarray(fold["y_outer_test"])

        assert y_outer_train.shape[0] == outer_train_idx.shape[0], (
            f"fold[{i}].y_outer_train length mismatch: {y_outer_train.shape[0]} vs {outer_train_idx.shape[0]}"
        )
        assert y_outer_test.shape[0] == outer_test_idx.shape[0], (
            f"fold[{i}].y_outer_test length mismatch: {y_outer_test.shape[0]} vs {outer_test_idx.shape[0]}"
        )
        assert np.array_equal(y_outer_train, y[outer_train_idx]), f"fold[{i}].y_outer_train != y[outer_train_idx]"
        assert np.array_equal(y_outer_test, y[outer_test_idx]),   f"fold[{i}].y_outer_test != y[outer_test_idx]"

        # Group leakage check (outer)
        if groups is not None:
            g_train = set(groups[outer_train_idx].tolist())
            g_test = set(groups[outer_test_idx].tolist())
            overlap_g = g_train.intersection(g_test)
            assert len(overlap_g) == 0, (
                f"Outer group leakage in fold[{i}] for groups {list(overlap_g)[:10]}."
            )

        # Inner splits
        assert "inner_splits" in fold, f"Fold {i} missing 'inner_splits'."
        inner_splits = fold["inner_splits"]
        assert isinstance(inner_splits, list), f"fold[{i}].inner_splits must be a list."
        assert len(inner_splits) == n_inner_splits, (
            f"Fold {i}: expected {n_inner_splits} inner folds, got {len(inner_splits)}."
        )

        outer_train_set = set(outer_train_idx.tolist())
        inner_val_counter = Counter()

        for j, inner in enumerate(inner_splits):
            inner_train_idx = _as_np_int(inner["inner_train_idx"], f"fold[{i}].inner[{j}].inner_train_idx")
            inner_val_idx   = _as_np_int(inner["inner_val_idx"],   f"fold[{i}].inner[{j}].inner_val_idx")

            _check_in_bounds(inner_train_idx, f"fold[{i}].inner[{j}].inner_train_idx")
            _check_in_bounds(inner_val_idx,   f"fold[{i}].inner[{j}].inner_val_idx")
            _check_disjoint(inner_train_idx, inner_val_idx, "inner_train_idx", "inner_val_idx")

            assert set(inner_train_idx.tolist()).issubset(outer_train_set), (
                f"fold[{i}].inner[{j}].inner_train_idx contains indices not in outer_train_idx."
            )
            assert set(inner_val_idx.tolist()).issubset(outer_train_set), (
                f"fold[{i}].inner[{j}].inner_val_idx contains indices not in outer_train_idx."
            )

            # Group leakage check (inner)
            if groups is not None:
                g_it = set(groups[inner_train_idx].tolist())
                g_iv = set(groups[inner_val_idx].tolist())
                overlap_g = g_it.intersection(g_iv)
                assert len(overlap_g) == 0, (
                    f"Inner group leakage in fold[{i}] inner[{j}] for groups {list(overlap_g)[:10]}."
                )

            inner_val_counter.update(inner_val_idx.tolist())

        if strict_partition_check:
            assert len(inner_val_counter) == len(outer_train_idx), (
                f"Fold {i}: inner_val coverage does not cover all outer_train samples. "
                f"covered={len(inner_val_counter)}, outer_train={len(outer_train_idx)}"
            )
            bad = [k for k, v in inner_val_counter.items() if v != 1]
            assert len(bad) == 0, (
                f"Fold {i}: some outer_train samples appear in inner_val != 1 times (e.g. {bad[:10]})."
            )

        test_counter.update(outer_test_idx.tolist())
        ...
        fold_summary = {
            ...
            "y_train": _counts_and_percents(outer_train_idx),
            "y_test": _counts_and_percents(outer_test_idx),
        }
        report["outer_folds"].append(fold_summary)

    # Outer test partition check
    if strict_partition_check:
        expected = num_trials
        assert len(test_counter) == n_samples, (
            f"Outer test coverage does not cover all samples. covered={len(test_counter)}, n_samples={n_samples}"
        )
        bad = [k for k, v in test_counter.items() if v != expected]
        assert len(bad) == 0, (
            f"Some samples appear in outer_test != {expected} times "
            f"(e.g. sample {bad[0]} appears {test_counter[bad[0]]} times)."
        )

    report["outer_test_coverage"] = {
        "n_unique_test_samples": len(test_counter),
        "min_times_in_test": min(test_counter.values()) if test_counter else None,
        "max_times_in_test": max(test_counter.values()) if test_counter else None,
    }

    if verbose:
        print("✅ Nested split plan passed validation.")
        print(f"- outer folds: {len(nested_splits)} (expected {expected_outer})")
        print(f"- outer test coverage: {report['outer_test_coverage']}")

        f0 = report["outer_folds"][0]
        print(f"- first outer fold sizes: train={f0['n_train']}, test={f0['n_test']}")

        tr_c = f0["y_train"]["counts"]
        tr_p = f0["y_train"]["percents"]
        te_c = f0["y_test"]["counts"]
        te_p = f0["y_test"]["percents"]

        print(f"- first outer fold y distribution (train): {tr_c} | percents: { {k: round(v, 2) for k, v in tr_p.items()} }")
        print(f"- first outer fold y distribution (test):  {te_c} | percents: { {k: round(v, 2) for k, v in te_p.items()} }")
...
def save_nested_cv_bundle(
    output_dir: Union[str, Path],
    *,
    nested_splits: List[Dict[str, Any]],
    prefix: str = "nested_cv",
    mode: str = "full",          # "full" or "plan_only"
    protocol: int = pickle.HIGHEST_PROTOCOL,
) -> Path:
    """
    Save a nested CV split bundle (nested_splits) to disk.

    This function is designed for nested CV plans that may optionally include
    attached artifacts from feature-selection pipelines (prune_out, rank_select_out, vif_out, etc.).

    Files written
    -------------
    Always:
      - {prefix}_splits.pkl
          Pickle of either the full nested_splits (mode="full") or a reduced plan-only
          representation (mode="plan_only").
      - {prefix}_manifest.json
          Small JSON summary: number of folds, inner folds, keys present, mode, etc.

    Parameters
    ----------
    output_dir
        Directory to write artifacts to (created if needed).
    nested_splits
        List of fold dictionaries returned by build_nested_cv_splits, possibly augmented
        with additional outputs (e.g., prune_out, rank_select_out, vif_out).
    prefix
        Filename prefix for outputs.
    mode
        - "full": save the full nested_splits object (largest, but complete).
        - "plan_only": save only indices + inner splits + y_outer_train/test + metadata keys.
          This avoids saving large matrices (e.g., prune_out["X"], rank_select_out["X"]).
    protocol
    ...
    if mode not in {"full", "plan_only"}:
        raise ValueError("mode must be 'full' or 'plan_only'")

    # Optionally reduce the object to a plan-only representation
    if mode == "plan_only":
        reduced: List[Dict[str, Any]] = []
        for fold in nested_splits:
            reduced.append(
                {
                    "trial": fold.get("trial"),
                    "outer_fold": fold.get("outer_fold"),
                    "model_selection": fold.get("model_selection"),
                    "outer_train_idx": fold.get("outer_train_idx"),
                    "outer_test_idx": fold.get("outer_test_idx"),
                    "inner_splits": fold.get("inner_splits"),
                    "y_outer_train": fold.get("y_outer_train"),
                    "y_outer_test": fold.get("y_outer_test"),
                }
            )
        obj_to_save = reduced
    else:
        obj_to_save = nested_splits

    # Pickle dump
    pkl_path = out / f"{prefix}_splits.pkl"
    with open(pkl_path, "wb") as f:
        pickle.dump(obj_to_save, f, protocol=protocol)

    # Build a small manifest (human-readable)
    n_outer = len(nested_splits)
    n_inner = None
    if n_outer > 0 and isinstance(nested_splits[0].get("inner_splits"), list):
        n_inner = len(nested_splits[0]["inner_splits"])

    # collect top-level keys frequency
    ...
    manifest = {
        ...
        "n_inner_folds_per_outer": n_inner,
        "top_level_keys_seen": sorted(key_counts.keys()),
        "top_level_key_counts": key_counts,
        "files": {
            "splits_pickle": pkl_path.name,
        },
    }
    ...
    print(f"✅ Saved nested CV bundle to: {out.resolve()}")
...
def load_nested_cv_bundle(
    ...
    *,
    ...
    """
    Load a nested CV bundle saved by `save_nested_cv_bundle`. Loads whatever files exist.

    Files (if present)
    ------------------
    - {prefix}_splits.pkl
        Pickled nested_splits object (either "full" or "plan_only" mode).
    - {prefix}_manifest.json
        Small JSON summary of the saved bundle (mode, keys seen, counts, etc.).

    Returns
    -------
    bundle : Dict[str, Any]
        Dictionary with keys:
          - "nested_splits": List[Dict[str, Any]] or None
    ...
    prefix
        Filename prefix used during saving.
    require
        Optional list of bundle keys that must be present, e.g.:
          require=["nested_splits"]
    """
    out = Path(output_dir)
    ...
    # Pickle: nested_splits
    pkl_path = out / f"{prefix}_splits.pkl"
    if pkl_path.exists():
        with open(pkl_path, "rb") as f:
            bundle["nested_splits"] = pickle.load(f)
...
def nested_cv_feature_selection(
    ...
    rank_select_cfg: Optional[Dict[str, Any]] = None,
    groups: Optional[np.ndarray] = None,
    store_mode: str = "full",
    prune_out_key: str = "prune_out",
    rank_select_out_key: str = "rank_select_out",
    verbose: bool = True,
) -> List[Dict[str, Any]]:
    """
    Attach per-fold feature selection outputs to a nested CV split plan.

    This function loops over the OUTER folds in `nested_splits` and, for each fold:

      1) Runs a pruning (filter) feature-selection stage on the OUTER-TRAIN subset
         defined by `fold["outer_train_idx"]`.

         The pruning stage is configured by `prune_cfg` and is expected to include
         steps such as constant/quasi-constant removal and collinearity pruning.

      2) Optionally runs a supervised rank→select stage immediately after pruning,
         using the SAME OUTER-TRAIN subset.

         The rank-select stage is configured by `rank_select_cfg` and uses:
           - X = prune_out["X"]
           - y = fold["y_outer_train"]
           - feature_names = prune_out["feature_names_selected"]

    The outputs are stored back into each fold dictionary:

      - fold[prune_out_key] = prune_out
      - fold[rank_select_out_key] = rank_select_out    (only if rank_select_cfg is provided)

    Notes
    -----
    - This function does NOT perform model training or hyperparameter tuning.
    - This function does NOT use inner CV splits; it only processes each outer fold.
    - If `rank_select_cfg` is provided, each fold MUST contain `y_outer_train` that
      corresponds to the same samples as `outer_train_idx`.
    - The function mutates `nested_splits` in-place and also returns it for convenience.

    Parameters
    ----------
    nested_splits
        List of dictionaries describing nested CV folds. Each fold dict must include
        at least "outer_train_idx" and should include "trial" / "outer_fold" for logging.
        If rank_select_cfg is provided, each fold must include "y_outer_train".
    X
        Full feature matrix of shape (n_samples, n_features). This will be sliced per fold
        using `outer_train_idx`.
    feature_names
        Feature names aligned to columns of `X`, length = n_features.
    prune_cfg
        Configuration for the pruning stage. Expected to include keys like
        "constant_cfg" and "collinearity_cfg" (depending on your implementation).
    rank_select_cfg
        Optional configuration for the rank-select stage (two-stage rank→aggregate→select).
        If None, only the pruning stage is run.
    groups
        Optional group labels aligned to rows of `X` (shape = (n_samples,)).
        If provided, each fold uses the outer-train subset of groups.
    store_mode
        Storage mode for pruning output. Use "full" to store the full prune output
        (including the pruned design matrix). If you later need rank-select, "full"
        is required because rank-select consumes prune_out["X"].
    prune_out_key
        Key name used to store pruning output inside each fold dict.
    rank_select_out_key
        Key name used to store rank-select output inside each fold dict.
    verbose
        If True, prints per-fold progress messages.

    Returns
    -------
    nested_splits
        The same list object as input, mutated in-place with per-fold outputs attached.
    """

    constant_cfg = prune_cfg.get("constant_cfg", {"enable": False})
    collinearity_cfg = prune_cfg.get("collinearity_cfg", {"enable": False})

    # Map feature name -> column index once (for minimal storage)
    ...
    for i, fold in enumerate(nested_splits):
        ...
        if verbose:
            print(f"[PRUNE] trial={trial} outer_fold={outer_fold} | X_train={X_train.shape}")

        # -----------------------------
        # 1) Prune on outer-train
        # -----------------------------
        prune_out = run_feature_selection_pipeline(
            X_raw=X_train,
            feature_names=feature_names,
            groups=groups_train,
            constant_cfg=constant_cfg,
            collinearity_cfg=collinearity_cfg,
        )

        if store_mode == "full":
            fold[prune_out_key] = prune_out
        elif store_mode == "minimal":
            selected_names = prune_out["feature_names_selected"]
            selected_idx = np.array([name_to_idx[n] for n in selected_names], dtype=int)
            fold[prune_out_key] = {
                "feature_names_selected": selected_names,
                "selected_feature_idx": selected_idx,
                "logs": prune_out.get("logs", {}),
            }
        else:
            raise ValueError("store_mode must be 'minimal' or 'full'")

        # -----------------------------
        # 2) Rank-select right after prune
        # -----------------------------
        if rank_select_cfg is not None:
            if "y_outer_train" not in fold:
                raise KeyError(
                    f"Fold {i} is missing 'y_outer_train'. "
                    "Rank-select must use the same outer-train labels that match X_train."
                )
            y_train = fold["y_outer_train"]

            rank_select_out = run_rank_select_pipeline(
                X=prune_out["X"],
                y=y_train,
                groups=groups_train,
                feature_names=prune_out["feature_names_selected"],
            ...
            if verbose:
                n_prune = prune_out["X"].shape[1]
                n_rank = rank_select_out["X"].shape[1]
                print(
                    f"[RANK_SELECT] trial={trial} outer_fold={outer_fold} | "
                    f"after prune={n_prune} feats -> after rank_select={n_rank} feats"
                )
...
def nested_cv_vif_pruning(
    nested_splits: List[Dict[str, Any]],
    vif_cfg: Dict[str, Any],
    groups: Optional[np.ndarray] = None,
    rank_select_out_key: str = "rank_select_out",   # where rank-select output is stored per fold (input to VIF)
    vif_out_key: str = "vif_out",                   # where to store the latest VIF output per fold
    store_history: bool = True,                     # if True, keep a record of multiple VIF runs (e.g., different thresholds)
    history_key: str = "vif_history",               # key to store the per-fold VIF run history
    verbose: bool = True,
) -> List[Dict[str, Any]]:
    """
    Attach VIF-based multicollinearity pruning outputs to a nested CV split plan.

    This function loops over the OUTER folds in `nested_splits` and, for each fold:
      - takes the fold's rank-select output (fold[rank_select_out_key]) as input
      - runs VIF-based iterative feature elimination
      - stores the latest VIF output in fold[vif_out_key]

    Rationale:
      VIF pruning is kept separate from upstream feature selection because the number of
      remaining features depends on the chosen VIF threshold. This enables threshold
      sweeps without recomputing earlier (more expensive) selection steps.

    Parameters
    ----------
    nested_splits
        List of fold dictionaries. Each fold must contain rank-select output under
        `rank_select_out_key`, with keys: "X" and "feature_names_selected".
    vif_cfg
        Configuration for VIF pruning. Expected keys:
          - "methods_config": must include {"vif": {"threshold": float, "cap": float}}
          - Optional: "N", "random_state", "parallelize", "n_jobs", "backend"
        Note: If `groups` is None, N is effectively ignored by your VIF implementation.
    groups
        Optional group labels aligned to rows of the original dataset. If provided and
        the fold contains "outer_train_idx", the outer-train subset of groups is used.
    rank_select_out_key
        Key name where the rank-select output dict is stored in each fold (input to VIF).
    vif_out_key
        Key name where the latest VIF output dict will be stored in each fold.
    store_history
        If True, appends a record of each VIF run into fold[history_key] so you can
        sweep thresholds without rerunning upstream steps.
    history_key
        Key name where per-fold VIF run history is stored (only used if store_history=True).
    verbose
        If True, prints per-fold feature counts before/after VIF.

    Returns
    -------
    nested_splits
        The same list object as input, mutated in-place with VIF outputs attached.
    """
    methods_config = vif_cfg["methods_config"]
    N = vif_cfg.get("N", 1000)
    random_state = vif_cfg.get("random_state", 42)
    parallelize = vif_cfg.get("parallelize", True)
    n_jobs = vif_cfg.get("n_jobs", -1)
    backend = vif_cfg.get("backend", "loky")

    # For tagging runs
    vif_threshold = methods_config.get("vif", {}).get("threshold", None)
    vif_cap = methods_config.get("vif", {}).get("cap", None)

    for i, fold in enumerate(nested_splits):
        trial = fold.get("trial", None)
        outer_fold = fold.get("outer_fold", None)

        if rank_select_out_key not in fold:
            raise KeyError(
                f"Fold {i} missing '{rank_select_out_key}'. "
                "Run rank-select first so VIF has an input matrix."
            )

        rank_select_out = fold[rank_select_out_key]
        X_in = rank_select_out["X"]
        feat_in = rank_select_out["feature_names_selected"]

        # Fold-level groups (only relevant if you later enable grouped VIF)
        groups_fold = None
        if groups is not None and "outer_train_idx" in fold:
            groups_fold = groups[fold["outer_train_idx"]]

        if verbose:
            print(
                f"[VIF] trial={trial} outer_fold={outer_fold} | "
                f"in_feats={X_in.shape[1]} | threshold={vif_threshold}"
            )

        out_vif = remove_multicollinearity_features(
            X=X_in,
            groups=groups_fold,
            methods_config=methods_config,
            N=N,
            feature_names=feat_in,
        ...
        # Store latest VIF output
        fold[vif_out_key] = out_vif

        # Optionally keep a history of runs (so you can sweep thresholds without reruns upstream)
        if store_history:
            if history_key not in fold:
                fold[history_key] = []
            fold[history_key].append(
                {

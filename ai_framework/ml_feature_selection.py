...
from typing import Any, Dict, List, Optional, Sequence, Tuple, Union, Type, Mapping, Iterable
...
def select_by_shuffling_stability(
    ...
    *,
    ...
    feature_names: Optional[Iterable[Any]] = None,
    validate_feature_names: bool = True,
    return_details: bool = True,
) -> Dict[str, Dict[str, Any]]:
    """
    Run Feature-engine's SelectByShuffling multiple times per model (stability runs) and
    aggregate feature importance (performance drift) and selection stability across:

      1) runs (n_runs) for each model, and
      2) models (overall summary).

    This function varies ONLY the SelectByShuffling(random_state=...) across runs.
    It does not intentionally vary estimator randomness. If your estimator is stochastic
    (e.g., RandomForest) and you want deterministic estimator fits, set the estimator's
    random_state when you create it.

    Parameters
    ----------
    X:
        Feature matrix. Either:
          - pandas.DataFrame (feature names taken from X.columns), or
          - numpy.ndarray (will be wrapped in a DataFrame using feature_names or autogenerated names).
    y:
        Target vector. Accepts pandas.Series, numpy.ndarray, or list-like. Will be flattened to 1D.
    models:
        Mapping of model name -> sklearn-compatible estimator (already instantiated).
        Example: {"rf": RandomForestClassifier(...), "lr": LogisticRegression(...)}
    n_runs:
        Number of stability runs per model (each run uses a different shuffling seed).
    random_state:
        Master seed used to generate the per-run shuffling seeds.
    cv:
        Cross-validation strategy passed to SelectByShuffling.
        - If None, defaults to StratifiedKFold(n_splits=5, shuffle=True, random_state=0)
        - Can also be an int or a sklearn CV splitter.
    feature_names:
        Optional feature names, used ONLY when X is a numpy array.
        If None and X is numpy, autogenerated names "0", "1", ..., "p-1" are used.
        If X is a DataFrame and validate_feature_names=True, provided feature_names must match X.columns.
    validate_feature_names:
        If True, validate provided feature_names against X.columns (when X is a DataFrame),
        validate length/uniqueness (when X is numpy).
    return_details:
        If True, returns per-run drift/kept matrices in addition to summary tables.

    Returns
    -------
    results:
        Dict keyed by model name plus "ALL_MODELS".

        For each model key:
          - results[model]["summary"]: DataFrame with columns:
              ["feature", "mean_drift", "std_drift", "median_drift", "keep_freq"]
          - (optional) results[model]["drifts"]: DataFrame (features x runs) of performance drift
          - (optional) results[model]["kept"]:   DataFrame (features x runs) of booleans kept/dropped

        For "ALL_MODELS":
          - results["ALL_MODELS"]["summary"]: DataFrame with columns:
              ["feature", "mean_drift_across_models", "keep_freq_across_models", "keep_freq_min_model"]
          - (optional) results["ALL_MODELS"]["wide"]: wide DataFrame used for aggregation
            (model__mean_drift / model__keep_freq columns)

    Notes
    -----
    - Higher drift means shuffling that feature hurts CV performance more => more important.
    - keep_freq is the fraction of runs in which the feature was kept by SelectByShuffling.
    """
    # -----------------------------
    # Normalize X and feature names
    # -----------------------------
    if isinstance(X, pd.DataFrame):
        X_df = X.copy()
        inferred_names: List[str] = [str(c) for c in X_df.columns.tolist()]

        if feature_names is not None and validate_feature_names:
            provided = [str(c) for c in list(feature_names)]
            if provided != inferred_names:
                raise ValueError(
                    "feature_names was provided but does not match X.columns.\n"
                    f"First 5 provided: {provided[:5]}\n"
                    f"First 5 X.columns: {inferred_names[:5]}"
                )

        feature_names_list = inferred_names
    else:
        X_arr = np.asarray(X)
        if X_arr.ndim != 2:
        ...
        if feature_names is None:
            feature_names_list = [str(i) for i in range(n_features)]
        else:
            feature_names_list = [str(c) for c in list(feature_names)]
            if validate_feature_names and len(feature_names_list) != n_features:
                raise ValueError(
                    f"feature_names length ({len(feature_names_list)}) must match "
                    f"number of columns in X ({n_features})."
                )

        if validate_feature_names and len(set(feature_names_list)) != len(feature_names_list):
            raise ValueError("feature_names contains duplicates; feature names must be unique.")

        X_df = pd.DataFrame(X_arr, columns=feature_names_list)
    ...
    if X_df.shape[0] != y_arr.shape[0]:
        raise ValueError(
            ...
            f"Got X: {X_df.shape[0]}, y: {y_arr.shape[0]}"
        )
    ...
    # -----------------------------
    # Generate per-run shuffling seeds
    # -----------------------------
    if n_runs <= 0:
        raise ValueError("n_runs must be a positive integer.")
    rng = np.random.default_rng(random_state)
    # sample unique seeds from a large space to avoid duplicates
    seeds = rng.choice(1_000_000, size=n_runs, replace=False).tolist()

    # -----------------------------
    # Run SBS per model & aggregate
    # -----------------------------
    results: Dict[str, Dict[str, Any]] = {}
    per_model_blocks: List[pd.DataFrame] = []

    for model_name, estimator in models.items():
        drifts_runs: List[pd.Series] = []
        kept_runs: List[pd.Series] = []

        for seed in seeds:
            sbs = SelectByShuffling(
                estimator=clone(estimator),  # fresh estimator each run
                cv=cv,
                random_state=int(seed),      # ONLY randomness we vary
            )
            sbs.fit(X_df, y_arr)

            drifts_runs.append(pd.Series(sbs.performance_drifts_, index=feature_names_list, name=seed))

            kept = set(feature_names_list) - set(sbs.features_to_drop_)
            kept_runs.append(pd.Series({c: (c in kept) for c in feature_names_list}, name=seed))

        drifts_df = pd.concat(drifts_runs, axis=1)  # rows=features, cols=runs
        kept_df = pd.concat(kept_runs, axis=1)      # rows=features, cols=runs

        summary = (
            pd.DataFrame({
                "mean_drift": drifts_df.mean(axis=1),
                "std_drift": drifts_df.std(axis=1),
                "median_drift": drifts_df.median(axis=1),
                "keep_freq": kept_df.mean(axis=1),
            })
            .sort_values(["mean_drift", "keep_freq"], ascending=False)
            .reset_index()
            .rename(columns={"index": "feature"})
        )

        if return_details:
            results[model_name] = {"summary": summary, "drifts": drifts_df, "kept": kept_df, "seeds": seeds}
        else:
            results[model_name] = {"summary": summary, "seeds": seeds}

        block = (
            summary.set_index("feature")[["mean_drift", "keep_freq"]]
            .rename(columns={
                "mean_drift": f"{model_name}__mean_drift",
                "keep_freq": f"{model_name}__keep_freq",
            })
        )
        per_model_blocks.append(block)

    # ---- Aggregate across models ----
    wide = pd.concat(per_model_blocks, axis=1).reindex(feature_names_list)

    mean_drift_cols = [c for c in wide.columns if c.endswith("__mean_drift")]
    keep_cols = [c for c in wide.columns if c.endswith("__keep_freq")]

    overall = (
        pd.DataFrame({
            "mean_drift_across_models": wide[mean_drift_cols].mean(axis=1),
            "keep_freq_across_models": wide[keep_cols].mean(axis=1),
            "keep_freq_min_model": wide[keep_cols].min(axis=1),
        })
        .sort_values(["mean_drift_across_models", "keep_freq_across_models"], ascending=False)
        .reset_index()
        .rename(columns={"index": "feature"})
    )

    if return_details:
        results["ALL_MODELS"] = {"summary": overall, "wide": wide, "seeds": seeds}
    else:
        results["ALL_MODELS"] = {"summary": overall, "seeds": seeds}

    return results

def plot_sbs_ranked_mean_with_annotations(
    sbs_results: Mapping[str, Mapping[str, Any]],
    *,
    top_n: int = 15,
    # ---- labels / aliasing ----
    ...
    x_tick_rotation: int = 70,
    # ---- line/marker styling ----
    line_color: str =  "#F14949",
    marker_color: Optional[str] = None,     # if None, uses line_color
    marker: str = "o",
    line_width: float = 2.0,
    marker_size: float = 8.0,
    # ---- annotation ----
    annotate: bool = True,
    annotate_decimals_mean: int = 4,
    annotate_decimals_keep: int = 2,
    annotate_font_size: Optional[float] = None,
    annotate_offset_points: tuple[float, float] = (0.0, 6.0),  # (dx, dy) in points
    # ---- toggles ----
    show_line: bool = True,
    show_points: bool = True,
) -> None:
    """
    Plot ranked mean drift for SelectByShuffling stability results.

    Produces:
      1) One plot for the aggregated ALL_MODELS summary, and
      2) One plot per model present in `sbs_results` (every key except "ALL_MODELS").

    Each plot shows the Top-N features by mean drift:
      - x-axis: feature (ranked, highest mean drift first)
      - y-axis: mean drift
      - optional: line connecting points in rank order
      - optional: scatter points
      - optional: annotation at each point with "(mean_drift, keep_freq)"

    Expected input structure
    ------------------------
    - sbs_results["ALL_MODELS"]["summary"] must be a pandas.DataFrame containing:
        ["feature", "mean_drift_across_models", "keep_freq_across_models", ...]
    - sbs_results[model]["summary"] must be a pandas.DataFrame containing:
        ["feature", "mean_drift", "keep_freq", ...]

    Parameters
    ----------
    sbs_results:
        Output from `select_by_shuffling_stability(...)`.

    top_n:
        Number of top-ranked features to plot per panel.

    method_alias:
        Optional mapping from internal model keys to display labels (titles).

    figsize, font_size, x_tick_rotation:
        Figure/label styling.

    line_color, marker_color, marker, line_width, marker_size:
        Control line and marker styling. marker_color defaults to line_color.

    sns_style: 
        seaborn style (default "whitegrid").
        
    annotate, annotate_decimals_mean, annotate_decimals_keep, annotate_font_size, annotate_offset_points:
        Controls annotation content and appearance.

    show_line, show_points:
        Toggle line connections and markers.
    ...
    # Defaults / validation
    # -------------------------
    if "ALL_MODELS" not in sbs_results:
        raise KeyError("sbs_results must contain key 'ALL_MODELS'.")
    if top_n <= 0:
        raise ValueError("top_n must be a positive integer.")
    if method_alias is None:
        method_alias = {}

    # validate alias uniqueness (like your barplot function)
    model_keys_for_alias = [k for k in sbs_results.keys() if k != "ALL_MODELS"]
    model_labels = [method_alias.get(m, m) for m in model_keys_for_alias]
    ...
    dx_pt, dy_pt = float(annotate_offset_points[0]), float(annotate_offset_points[1])
    ...
    def _require_cols(df: pd.DataFrame, cols: list[str], where: str) -> None:
        ...
        if missing:
            raise KeyError(f"Missing required columns in {where}: {missing}. Available: {list(df.columns)}")


    # Seaborn styling (PDP-like)
    sns.set(style=sns_style)

    def _annotate_points(ax: plt.Axes, x_vals, y_vals, labels) -> None:
    ...
    def _plot_one(
        ...
        *,
        ...
        _require_cols(df, ["feature", mean_col, keep_col], where=title)

        dfp = df[["feature", mean_col, keep_col]].copy()
        dfp = dfp.sort_values(mean_col, ascending=False).head(top_n)

        # preserve rank order on x-axis
        dfp["feature"] = pd.Categorical(dfp["feature"], categories=dfp["feature"], ordered=True)
        ...
        if show_points and not show_line:
            sns.scatterplot(
                data=dfp,
                x="feature",
                y=mean_col,
                ax=ax,
                color=marker_color,
                s=float(marker_size) ** 2,  # seaborn scatter uses area
            )

        if show_points and show_line:
            # overlay points so marker_color can differ from line_color
            sns.scatterplot(
                data=dfp,
                x="feature",
                y=mean_col,
        ...
        ax.set_ylabel(mean_col, fontsize=font_size, fontweight="bold")

        ax.tick_params(axis="x", labelsize=font_size, rotation=x_tick_rotation)
        ...
        if annotate:
            labels = [
                f"({m:.{annotate_decimals_mean}f}, {k:.{annotate_decimals_keep}f})"
                for m, k in zip(dfp[mean_col].to_numpy(), dfp[keep_col].to_numpy())
            ]
            _annotate_points(ax, dfp["feature"], dfp[mean_col], labels)
    ...
    # ALL_MODELS plot
    # -------------------------
    overall = sbs_results["ALL_MODELS"].get("summary")
    if not isinstance(overall, pd.DataFrame):
        raise TypeError("sbs_results['ALL_MODELS']['summary'] must be a pandas DataFrame.")

    _plot_one(
        overall,
        mean_col="mean_drift_across_models",
        keep_col="keep_freq_across_models",
        title=f"ALL_MODELS: Top {top_n} ranked by mean drift (annotated: mean, keep_freq)",
    )

    # -------------------------
    # Per-model plots (all models present)
    # -------------------------
    for model_name in [k for k in sbs_results.keys() if k != "ALL_MODELS"]:
        dfm = sbs_results[model_name].get("summary")
        if not isinstance(dfm, pd.DataFrame):
            raise TypeError(f"sbs_results['{model_name}']['summary'] must be a pandas DataFrame.")

        disp = method_alias.get(model_name, model_name)
        _plot_one(
            dfm,
            mean_col="mean_drift",
            keep_col="keep_freq",
            title=f"{disp}: Top {top_n} ranked by mean drift (annotated: mean, keep_freq)",
        )

def get_top_features_from_sbs(
    sbs_results: Mapping[str, Mapping[str, Any]],
    *,
    top_n: Optional[int] = 20,
    per_model_top_n: Optional[Mapping[str, int]] = None,
    include_all_models: bool = False,
    all_models_top_n: Optional[int] = None,
) -> Dict[str, List[str]]:
    """
    Return top feature names as lists for each model in sbs_results, with support for
    model-specific cutoffs.

    How selection count is determined
    --------------------------------
    For each model:
      - If per_model_top_n is provided and contains the model, use that value.
      - Else use top_n.
      - If both are missing (top_n is None and model not in per_model_top_n), raise an error.

    For ALL_MODELS (optional):
      - Use all_models_top_n if provided, else top_n (must resolve to an int).
    ...
    top_n:
        Default number of features to return per model. Can be None if you provide
        per_model_top_n for every model you want returned.

    per_model_top_n:
        Optional mapping like {"lr": 4, "xgb": 7}. If a model is missing from this mapping,
        it falls back to top_n.

    include_all_models:
        If True, also return an "ALL_MODELS" list.

    all_models_top_n:
        Optional separate cutoff for "ALL_MODELS". If None, uses top_n.

    Returns
    -------
    Dict[str, List[str]]
        Keys are model names (and optionally "ALL_MODELS"), values are lists of feature names.
    """
    if per_model_top_n is None:
        per_model_top_n = {}

    if top_n is not None and top_n <= 0:
        raise ValueError("top_n must be a positive integer or None.")

    for k, v in per_model_top_n.items():
        if v <= 0:
            raise ValueError(f"per_model_top_n['{k}'] must be a positive integer. Got {v}.")

    out: Dict[str, List[str]] = {}

    model_names = [k for k in sbs_results.keys() if k != "ALL_MODELS"]

    for model_name in model_names:
        df = sbs_results[model_name].get("summary")
        if not isinstance(df, pd.DataFrame):
            raise TypeError(f"sbs_results['{model_name}']['summary'] must be a pandas DataFrame.")

        if "feature" not in df.columns or "mean_drift" not in df.columns:
            raise KeyError(f"Expected columns ['feature','mean_drift'] in {model_name} summary.")

        n = per_model_top_n.get(model_name, top_n)
        if n is None:
            raise ValueError(
                f"No top-N specified for model '{model_name}'. "
                "Provide top_n or per_model_top_n for this model."
            )

        out[model_name] = (
            df.sort_values("mean_drift", ascending=False)
              .head(int(n))["feature"]
              .astype(str)
              .tolist()
        )
    ...
    if include_all_models:
        df_all = sbs_results.get("ALL_MODELS", {}).get("summary")
        if not isinstance(df_all, pd.DataFrame):
            raise TypeError("sbs_results['ALL_MODELS']['summary'] must be a pandas DataFrame.")

        if "feature" not in df_all.columns or "mean_drift_across_models" not in df_all.columns:
            raise KeyError("Expected columns ['feature','mean_drift_across_models'] in ALL_MODELS summary.")

        n_all = all_models_top_n if all_models_top_n is not None else top_n
        if n_all is None:
            raise ValueError(
                "include_all_models=True requires either all_models_top_n or top_n (not None)."
            )
        if int(n_all) <= 0:
            raise ValueError("all_models_top_n must be a positive integer when provided.")

        out["ALL_MODELS"] = (
            df_all.sort_values("mean_drift_across_models", ascending=False)
                 .head(int(n_all))["feature"]
                 .astype(str)
                 .tolist()
        )

# nested_ml_tuning_calib_interpret.py
...
def prepare_precomputed_nested_cv_folds(
    X: np.ndarray,
    y: np.ndarray,
    split_plan: List[Dict[str, Any]],
    groups: Optional[np.ndarray] = None,
    carry_keys: Optional[List[str]] = None,
    final_feature_source: str = "vif_out",
    final_feature_key: str = "feature_names_selected",
) -> List[Dict[str, Any]]:
    """
    Materialize a precomputed nested split plan (outer + inner GLOBAL indices)
    into actual X/y (and optionally groups) arrays per fold.

    Adds per-OUTER-fold "final_feature_names" (and optionally "final_feature_idx")
    extracted from `rec[final_feature_source][final_feature_key]`, e.g.
    rec["vif_out"]["feature_names_selected"].

    NO tuning, NO model fitting.

    Parameters
    ----------
    X, y
        Full dataset arrays. Indices in `split_plan` must refer to these.
    split_plan
        List of dicts like cv_bundle['nested_splits'] entries.
    groups
        Optional group labels aligned to X/y.
    carry_keys
        Keys to carry through from split_plan into the output (e.g. feature-selection artifacts).
    final_feature_source
        Which top-level dict key contains the final feature-selection output (default "vif_out").
    final_feature_key
        Which key under `final_feature_source` holds the selected feature names
        (default "feature_names_selected").

    Returns
    -------
    folds : List[Dict[str, Any]]
        One dict per outer fold, containing:
          - outer indices + X/y slices
          - inner folds (indices + X/y slices)
          - per-outer-fold final_feature_names (+ metadata)
          - optionally carried artifacts
    """
    ...
    for rec in split_plan:
        outer_train_idx = rec["outer_train_idx"]
        outer_test_idx  = rec["outer_test_idx"]

        # Outer slices
        X_outer_train = X[outer_train_idx]
        y_outer_train = rec.get("y_outer_train", y[outer_train_idx])

        X_outer_test  = X[outer_test_idx]
        y_outer_test  = rec.get("y_outer_test", y[outer_test_idx])

        # Optional group slices
        groups_outer_train = groups[outer_train_idx] if groups is not None else None
        groups_outer_test  = groups[outer_test_idx]  if groups is not None else None

        # Inner slices (GLOBAL indices)
        inner_materialized: List[Dict[str, Any]] = []
        for inner_rec in rec["inner_splits"]:
            tr_idx = inner_rec["inner_train_idx"]
            va_idx = inner_rec["inner_val_idx"]

            inner_materialized.append(
                {
                    "inner_fold": inner_rec["inner_fold"],
                    "inner_train_idx": tr_idx,
                    "inner_val_idx": va_idx,
                    "X_inner_train": X[tr_idx],
                    "y_inner_train": y[tr_idx],
                    "X_inner_val": X[va_idx],
                    "y_inner_val": y[va_idx],
                    "groups_inner_train": groups[tr_idx] if groups is not None else None,
                    "groups_inner_val": groups[va_idx] if groups is not None else None,
                }
            )

        # Pull final features for THIS OUTER FOLD (if present)
        final_feature_names = None
        if final_feature_source in rec and isinstance(rec[final_feature_source], dict):
            final_feature_names = rec[final_feature_source].get(final_feature_key, None)

        out: Dict[str, Any] = {
            "trial": rec.get("trial", None),
            "outer_fold": rec.get("outer_fold", None),
            "model_selection": rec.get("model_selection", None),
            ...
            "groups_outer_test": groups_outer_test,

            "inner_splits": inner_materialized,

            # Per-outer-fold final feature set
            "final_feature_source": final_feature_source,
            "final_feature_key": final_feature_key,
            "final_feature_names": final_feature_names,
            "final_feature_count": (len(final_feature_names) if final_feature_names is not None else None),
        }

        # Carry through any requested artifacts (e.g., prune_out, rank_select_out, vif_out, vif_history)
        for k in carry_keys:
            if k in rec:
                out[k] = rec[k]
...
# ---------------------------------------------------------------------
# 1) Build an inner-CV iterable (LOCAL indices) from precomputed GLOBAL splits
# ---------------------------------------------------------------------
def inner_cv_from_precomputed_splits(
    fold: Dict[str, Any]
) -> List[Tuple[np.ndarray, np.ndarray]]:
    """
    Convert a fold's precomputed GLOBAL inner splits into LOCAL index splits
    suitable for sklearn.model_selection.cross_validate(cv=...).

    Required keys in `fold`:
      - "outer_train_idx": np.ndarray (GLOBAL indices)
      - "inner_splits": list of dicts, each with:
          - "inner_train_idx": np.ndarray (GLOBAL)
          - "inner_val_idx": np.ndarray (GLOBAL)

    Returns
    -------
    cv_splits_local : list of (train_local_idx, val_local_idx)
        Each array indexes rows of the OUTER-TRAIN matrix (X_train) only.
    """
    outer_train_idx = fold["outer_train_idx"]
    pos = {g: i for i, g in enumerate(outer_train_idx)}  # GLOBAL -> LOCAL within outer train

    cv_splits_local: List[Tuple[np.ndarray, np.ndarray]] = []
    for inner in fold["inner_splits"]:
        tr_global = inner["inner_train_idx"]
        va_global = inner["inner_val_idx"]

        tr_local = np.array([pos[g] for g in tr_global], dtype=int)
        va_local = np.array([pos[g] for g in va_global], dtype=int)

        cv_splits_local.append((tr_local, va_local))

    return cv_splits_local
# ---------------------------------------------------------------------
# 2) Run nested CV for a single model using PRECOMPUTED splits + PER-OUTER-FOLD final features
# ---------------------------------------------------------------------
def run_nested_cv_for_model_precomputed(
    model_name: str,
    X: np.ndarray,
    y: np.ndarray,
    cfg: Dict[str, Any],
    folds: List[Dict[str, Any]],              # output of prepare_precomputed_nested_cv_folds(...)
    feature_names: List[str],                 # canonical names aligned with columns of X
    *,
    max_features: Optional[int] = None,       
) -> List[Dict[str, Any]]:
    """
    Run nested CV for a single model using a **precomputed nested split plan**.

    This is the “precomputed-splits” version of `run_nested_cv_for_model(...)`:
      - It does **not** create any CV splitters.
      - It iterates over the already-prepared `folds` objects (outer folds),
        and for each outer fold it uses that fold’s stored inner split plan
        to run Optuna hyperparameter tuning (inner CV), then fits/evaluates
        the final model on the outer train/test split.

    Feature selection integration (per outer fold)
    ----------------------------------------------
    Each `fold` includes a per-outer-fold final feature set (e.g. the final
    output of your feature-selection pipeline, such as VIF pruning). This
    function applies that feature set before any tuning/training:

      - `fold["final_feature_names"]` defines which columns of X to use for
        that outer fold.
      - Those names are mapped to column indices using the provided
        `feature_names` list (canonical, aligned to X columns).
      - Then X_train/X_test are subset to those columns.

    Optional feature cap (max_features)
    -----------------------------------
    Because different outer folds may yield slightly different counts of
    selected features, you can optionally enforce a global maximum:

      - If `max_features is None` (default): use the full per-fold selected set.
      - If `max_features` is an int >= 1: use at most the first `max_features`
        features from `fold["final_feature_names"]` for that fold.
      - If a fold has fewer than `max_features`, it simply uses all available
        features (this is allowed/expected).

    IMPORTANT: This does not “reselect” features — it only truncates the
    fold’s already-determined feature ordering.

    Parameters
    ----------
    model_name : str
        Key in `cfg["models"]` specifying which model to tune/train.

    X : np.ndarray
        Full feature matrix (n_samples, n_features). Used for slicing and
        column subsetting. Must align with `feature_names` column order.

    y : np.ndarray
        Full label vector (n_samples,). Used for training and evaluation.

    cfg : Dict[str, Any]
        Configuration dictionary. Must include:
          - cfg["optuna"]["n_inner_trials"], cfg["optuna"]["direction"]
          - cfg["metric"]["scoring"]
          - cfg["models"][model_name]["estimator_cls"]
          - cfg["models"][model_name]["params"] (callable(trial)->dict)

    folds : List[Dict[str, Any]]
        Output of `prepare_precomputed_nested_cv_folds(...)`. Each fold dict must include:
          - "trial", "outer_fold"
          - "outer_train_idx", "outer_test_idx"  (GLOBAL indices)
          - "X_outer_train", "y_outer_train", "X_outer_test", "y_outer_test"
          - "inner_splits" (GLOBAL indices describing inner train/val splits)
          - "final_feature_names" (per outer fold; e.g., from vif_out)

    feature_names : List[str]
        Canonical feature names aligned with columns of X. Used to map
        fold["final_feature_names"] -> column indices.

    max_features : Optional[int], default=None
        If provided, cap the number of per-fold selected features used for
        training/tuning. Folds with fewer features than `max_features` are
        unchanged (they use all available features).

    Returns
    -------
    results : List[Dict[str, Any]]
        One dictionary per outer fold (same overall schema as
        `run_nested_cv_for_model(...)`), including:
          - inner CV score arrays + summary stats (from Optuna best trial)
          - outer train/test metrics
          - best_params
          - final_model
          - bookkeeping indices and predictions
          - feature bookkeeping:
              * "final_feature_names" / "final_feature_idx" (ACTUALLY USED; capped if requested)
              * "final_feature_names_full" / "final_feature_idx_full" (original per-fold list before cap)
              * "max_features", "n_features_full", "n_features_used"
    """
    opt_cfg = cfg["optuna"]
    ...
    if max_features is not None:
        if not isinstance(max_features, int):
            raise TypeError(f"max_features must be int or None; got {type(max_features)}")
        if max_features <= 0:
            raise ValueError(f"max_features must be >= 1 when provided; got {max_features}")

    # Map feature name -> column index once
    ...
    print("=" * 200)
    print(f"\n=== Model: {model_name} | PRECOMPUTED SPLITS ===")
    print("=" * 200)

    for i, fold in enumerate(folds, start=1):
        print(
            f"Outer fold {i}/{total_outer_folds} "
            f"(trial {fold.get('trial')}, fold {fold.get('outer_fold')})"
        )

        # --------------------------------------------------------------
        # Outer train/test split (already materialized by you)
        # --------------------------------------------------------------
        X_train = fold["X_outer_train"]
        y_train = fold["y_outer_train"]
        X_test = fold["X_outer_test"]
        y_test = fold["y_outer_test"]

        # --------------------------------------------------------------
        # Apply PER-OUTER-FOLD final features (+ optional cap)
        # --------------------------------------------------------------
        final_feature_names_full = fold.get("final_feature_names", None)
        if final_feature_names_full is None:
            raise KeyError(
                "fold is missing 'final_feature_names'. "
                "Ensure prepare_precomputed_nested_cv_folds(...) was run with final_feature_source='vif_out' "
                "and that your split_plan contains rec['vif_out']['feature_names_selected']."
            )

        # Convert feature names -> indices (FULL set first)
        try:
            final_feature_idx_full = np.array([feat_to_idx[n] for n in final_feature_names_full], dtype=int)
        except KeyError as e:
            missing = str(e)
            raise KeyError(
                f"final_feature_names contains a name not found in feature_names: {missing}"
            )

        # Cap features if requested (fold may have fewer — that's OK)
        if max_features is None:
            final_feature_names = list(final_feature_names_full)
            final_feature_idx = final_feature_idx_full
        else:
            final_feature_names = list(final_feature_names_full[:max_features])
            final_feature_idx = final_feature_idx_full[:max_features]

        # Subset columns for this outer fold
        X_train = X_train[:, final_feature_idx]
        X_test = X_test[:, final_feature_idx]

        # --------------------------------------------------------------
        # Build INNER CV from precomputed splits (LOCAL indices)
        # --------------------------------------------------------------
        inner_cv = inner_cv_from_precomputed_splits(fold)

        # Splits already encode grouping constraints if you used them upstream
        groups_train = None
        ...
        # --------------------------------------------------------------
        # Collect all information for this outer fold (same schema)
        # --------------------------------------------------------------
        results.append(
            {
                "model_name": model_name,
                ...
                # inner CV stats (directly from best_trial, same as before)
                ...
                # features used (capped)
                "final_feature_names": final_feature_names,
                "final_feature_idx": final_feature_idx,

                # optional transparency
                "final_feature_names_full": list(final_feature_names_full),
                "final_feature_idx_full": final_feature_idx_full,
                "max_features": max_features,
                "n_features_full": len(final_feature_names_full),
                "n_features_used": len(final_feature_names),
            }
        )
...
def run_nested_permutation_importance(
    all_results: Dict[str, List[Dict[str, Any]]],
    X: np.ndarray,
    y: np.ndarray,
    *,
    scoring: str,
    scoring_label: Optional[str] = None,   # only for plot labeling
    n_repeats: int = 10,
    random_state: int = 42,
    n_jobs: int = -1,
    top_n: int = 15,
    ...
    font_size: int = 13,
    color: str = "#6af3d3",
    x_tick_rotation: float = 25,
    method_alias: Optional[Dict[str, str]] = None,
) -> Dict[str, List[Dict[str, Any]]]:
    """
    Compute permutation importance per OUTER fold in a nested-CV result dict,
    where EACH fold may have a different feature set (final_feature_idx/names).

    This:
      - loops model -> folds
      - slices X_outer_test to that fold’s selected columns
      - runs sklearn.inspection.permutation_importance on OUTER TEST only
      - stores results back into each fold dict
      - optionally plots one barplot per fold using your existing plot function

    Requirements (per fold dict in all_results[model_name]):
      - "final_model"
      - "outer_test_idx"
      - "final_feature_idx"
      - "final_feature_names"
    ...
    Returns
    -------
    all_results : same object, mutated in-place (and also returned for convenience)
    """
    ...
    for model_name, fold_list in all_results.items():
        ...
        for r in fold_list:
            # Basic checks (fail fast with a clear message)
            for req in ("final_model", "outer_test_idx", "final_feature_idx", "final_feature_names"):
                if req not in r:
                    raise KeyError(
                        f"Missing key '{req}' in fold result for model '{model_name}'. "
            ...
            test_idx = r["outer_test_idx"]
            feat_idx = r["final_feature_idx"]

            # Slice OUTER-TEST rows + fold-specific feature columns
            X_test_fold = X[test_idx][:, feat_idx]
            y_test_fold = y[test_idx]

            # Compute permutation importance on OUTER TEST only
            ...
            # Store results back into the fold dict
            r["permutation_importances"] = pi.importances  # (n_features_fold, n_repeats)
            r["permutation_importances_mean"] = pi.importances_mean
            r["permutation_importances_std"] = pi.importances_std
            ...
            # Optional plotting: 1 plot per fold
            if plot:
                # Give this fold a unique key so your existing plotter can handle it
                fold_key = f"{model_name}_t{trial}_f{outer_fold}"
                fold_combined_importances = {fold_key: pi.importances}

                display_name = method_alias.get(model_name, model_name)
                fold_alias = {fold_key: f"{display_name} (trial {trial}, outer {outer_fold})"}

                plot_permutation_importances_barplot(
                    combined_importances=fold_combined_importances,
                    model_name=None,  # only one entry, so plot it
                    feature_names=r["final_feature_names"],
                    top_n=top_n,
                    scoring=scoring_label,  # label only
                    figsize=figsize,
                    font_size=font_size,
                    color=color,
                    x_tick_rotation=x_tick_rotation,
                    method_alias=fold_alias,
                )

    return all_results


def attach_pdp_raw_grids_per_fold(
    all_results,
    scaler_bundle,
    *,
    source="test",
    scaler_paths=(("preproc", "scaler"), ("scaler",), ("x_scaler",), ("feature_scaler",)),
):
    """
    Adds per-fold raw-grid values:
      r[f"pdp_grid_values_raw_{source}"] = list[np.ndarray] (one per feature)

    Uses each fold's selected feature indices (final_feature_idx) to pick the
    correct per-feature mean_/scale_ from a StandardScaler.

    Expects:
      - fold dict has: "final_feature_idx", f"pdp_grid_values_{source}"
      - scaler exists at scaler_bundle[path...], e.g. ("preproc","scaler")
      - scaler has attributes mean_ and scale_ (StandardScaler-like)
    """
    ...
    # Find scaler by trying candidate paths
    scaler = None
    found_path = None
    for path in scaler_paths:
        cand = _get_by_path(scaler_bundle, path)
        if cand is not None:
            scaler = cand
            found_path = path
            break
    ...
    if scaler is None:
        raise KeyError(
            f"Could not find a scaler at any of these paths: {scaler_paths}. "
            f"Top-level keys in scaler_bundle: {list(scaler_bundle.keys()) if isinstance(scaler_bundle, dict) else type(scaler_bundle)}"
        )

    if not (hasattr(scaler, "mean_") and hasattr(scaler, "scale_")):
        raise TypeError(
            f"Scaler found at path {found_path} is not StandardScaler-like "
            f"(missing mean_/scale_). Got: {type(scaler)}"
        )
    ...
    out_key = f"pdp_grid_values_raw_{source}"

    for model_name, fold_list in all_results.items():
        for r in fold_list:
            if grid_key not in r:
                continue  # PDP not computed for this fold/source

            feat_idx = np.asarray(r["final_feature_idx"], dtype=int)
            grids_scaled = r[grid_key]  # list of arrays (fold-local feature order)

            if len(grids_scaled) != len(feat_idx):
                raise ValueError(
                    f"Mismatch in fold '{model_name}' trial={r.get('trial')} outer_fold={r.get('outer_fold')}: "
                    f"len({grid_key})={len(grids_scaled)} but len(final_feature_idx)={len(feat_idx)}"
                )

            grids_raw = []
            for local_j, g_scaled in enumerate(grids_scaled):
                global_j = int(feat_idx[local_j])
                g_scaled = np.asarray(g_scaled)
                g_raw = g_scaled * scale_[global_j] + mean_[global_j]
                grids_raw.append(g_raw)

            r[out_key] = grids_raw
            r[f"pdp_raw_grid_meta_{source}"] = {"scaler_path": found_path}

    return all_results

def run_nested_pdp_per_fold(
    all_results: Dict[str, List[Dict[str, Any]]],
    X: np.ndarray,
    y: np.ndarray,  # kept for symmetry; not used by partial_dependence directly
    *,
    data_source: Literal["train", "test", "both"] = "test",
    grid_resolution: int = 100,
    percentiles: Tuple[float, float] = (0.0, 1.0),
    centered: bool = False,  # kept for API symmetry (partial_dependence has no centered arg)
    n_points: int = 20,      # kept if you later want cross-fold interpolation; not used here
    discrete_threshold: int = 10,  # same
    scaler_bundle: Optional[Dict[str, Any]] = None,  # optional; only used if you add raw-grid conversion later
    plot: bool = False,
    top_features: Optional[int] = None,  # if you want only first N features (or use your own selection)
    verbose: bool = True,
) -> Dict[str, List[Dict[str, Any]]]:
    """
    Compute PDPs fold-by-fold for nested CV results where EACH fold has a different feature set.

    Requires per fold dict:
      - "final_model"
      - "outer_train_idx"
      - "outer_test_idx"
      - "final_feature_idx"
    ...
    Stores into each fold dict (depending on data_source):
      - "pdp_grid_values_test": list[np.ndarray] length n_features_fold
      - "pdp_average_values_test": list[np.ndarray] length n_features_fold
      (and/or *_train)

    Notes
    -----
    - This does NOT aggregate across folds (because feature sets differ).
    - Plotting is optional; you can plot downstream using the stored arrays.
    """
    # quick validation helper
    def _require_keys(r: Dict[str, Any], model_name: str):
        req = ("final_model", "outer_train_idx", "outer_test_idx", "final_feature_idx", "final_feature_names")
        missing = [k for k in req if k not in r]
        if missing:
            raise KeyError(
                f"Missing keys {missing} in fold dict for model '{model_name}'. "
                f"Available keys: {list(r.keys())}"
            )

    # which splits to compute on
    sources: List[str]
    if data_source == "both":
        sources = ["train", "test"]
    else:
    ...
    # progress
    total_folds = sum(len(v) for v in all_results.values())
    it = tqdm(total=total_folds, desc="Nested PDP per fold", disable=not verbose)

    for model_name, folds in all_results.items():
        for r in folds:
            _require_keys(r, model_name)

            model = r["final_model"]
            feat_idx = np.asarray(r["final_feature_idx"], dtype=int)
            feat_names = list(r["final_feature_names"])

            if top_features is not None:
                feat_idx = feat_idx[:top_features]
                feat_names = feat_names[:top_features]

            for src in sources:
                if src == "train":
                    row_idx = r["outer_train_idx"]
                else:
                    row_idx = r["outer_test_idx"]

                X_src = X[row_idx][:, feat_idx]

                # compute PDP per feature (1D PDP)
                grid_values_list: List[np.ndarray] = []
                avg_values_list: List[np.ndarray] = []

                for j in range(X_src.shape[1]):
                    # partial_dependence expects features in the COLUMN INDEX SPACE OF X_src
                    ...
                    # pd_out["grid_values"] is a list; for 1 feature, it has one 1D array
                    gv = np.asarray(pd_out["grid_values"][0])
                    av = np.asarray(pd_out["average"][0])  # shape (n_grid,)

                    grid_values_list.append(gv)
                    avg_values_list.append(av)

                # store on fold dict
                r[f"pdp_feature_names_{src}"] = feat_names
                r[f"pdp_grid_values_{src}"] = grid_values_list
                r[f"pdp_average_values_{src}"] = avg_values_list
                r[f"pdp_meta_{src}"] = {
                    "grid_resolution": grid_resolution,
                    "percentiles": percentiles,
                    "data_source": src,
                ...
                # optional quick plot per fold (simple)
                if plot:
                    import matplotlib.pyplot as plt

                    trial = r.get("trial", None)
                    outer_fold = r.get("outer_fold", None)
                    for fname, gv, av in zip(feat_names, grid_values_list, avg_values_list):
                        plt.figure(figsize=(8, 4))
                        plt.plot(gv, av, marker="o")
                        plt.title(f"PDP ({model_name}) trial {trial} outer {outer_fold}\nFeature: {fname}")
                        plt.xlabel("Feature value")
                        plt.ylabel("Partial dependence (average)")
                        plt.tight_layout()
                        plt.show()

...
from ml_project_state import ml_project_states
...
async def on_chat_start() -> None:
    ...
    await cl.Message(content=_render_startup_greeting()).send()
...
async def on_message(message: cl.Message) -> None:
    ...
    performance.mark("message_received")
    turn_failed = False

    try:
        api_key = cl.user_session.get(OPENAI_API_KEY_SESSION_KEY)
        if not api_key:
            await _send_completed_message(
                "Start a new chat and enter an API key first.", performance
            )
            return
        ...
        try:
            ...
            await _send_completed_message(
                "I could not access the conversation session. Please start a new chat.",
                performance,
            )
            return
        ...
        performance.set_attachment_context(
            attachment_processed=bool(attachments),
            native_file_input_included=bool(uploaded_attachments),
        )
        if attachments:
            performance.stage_ended(
                "attachment_processing",
                "attachment_processing_end",
                count=len(attachments),
                native_file_count=len(uploaded_attachments),
            )
        ...
        if attachments and not user_message and not uploaded_attachments:
            if local_status_notes:
                await _send_completed_message(
                    "\n\n".join(local_status_notes), performance
                )
            return
        if not user_message:
            if uploaded_attachments:
                ...
                await _send_completed_message(
                    "Please enter a message or attach a supported file.", performance
                )
                return

        response_prefix = "\n\n".join(local_status_notes)
        if response_prefix:
            response_prefix += "\n\n"
        response_message = cl.Message(content=response_prefix)

        try:
            session = conversation_sessions.get_session(chainlit_session_id)
            workspace = tabular_workspaces.get(chainlit_session_id)
            agent = ConversationalAgent(api_key=api_key)
            async for token in agent.stream_response(
                user_message,
                session=session,
                file_ids=[
                    attachment.openai_file_id for attachment in uploaded_attachments
                ],
                runtime_context=MLAgentContext(
                    session_id=chainlit_session_id,
                    tabular_workspace=workspace,
                    ml_project_state=ml_project_states.get_or_create(
                        chainlit_session_id,
                        workspace,
                    ),
                ),
                ...
                performance.mark_first_visible_token()

            await response_message.send()
            performance.mark("chainlit_response_completed")
        except Exception as exc:
            turn_failed = True
            _log_sanitized_exception("Conversational agent request failed", exc, api_key)
            error_message = "I could not complete that response. Please try again."
            if response_message.content.strip():
                response_message.content = (
                    f"{response_message.content.rstrip()}\n\n{error_message}"
                )
            else:
        ...
        performance.finish(failed=turn_failed)
...
async def _send_completed_message(
    ...
    """Send one non-streamed turn response and record UI completion."""

    await cl.Message(content=content).send()
    performance.mark("chainlit_response_completed")


def _render_startup_greeting() -> str:
    """Return the concise greeting shown after API-key validation."""

    return "\n".join(
        [
            "## Your AI Framework Assistant is ready",
            "",
...
async def on_chat_end() -> None:
    ...
    try:
        ...
        ml_project_states.remove_session(chainlit_session_id)
    except Exception:
...
def _prepare_local_tabular_workspaces(
    ...
    for attachment_index, attachment in enumerate(attachments, start=1):
        ...
        mime_type = str(raw_mime_type) if raw_mime_type else None
        pandas_stage_key = f"local_pandas_load:{attachment_index}"
        performance.stage_started(
            pandas_stage_key,
            "local_pandas_load_start",
            attachment_index=attachment_index,
        )
        try:
        ...
        performance.stage_ended(
            ...
            "local_pandas_load_end",
            attachment_index=attachment_index,
            status="loaded" if result.workspace is not None else "not_loaded",
        )
        if not result.is_supported_tabular_file:
            continue
        if result.workspace is not None:
            project_state = ml_project_states.get_or_create(
                chainlit_session_id,
                current_workspace,
            )
            if project_state.accepts_external_validation_upload():
                project_state.attach_external_validation_workspace(result.workspace)
                status_notes.append(
                    _render_external_validation_workspace_success(result.workspace)
                )
                continue
            ...
            ml_project_states.reset_for_workspace(chainlit_session_id, result.workspace)
            status_notes.append(
...
def _render_local_workspace_success(
    ...
    """Render a concise, user-facing table-load confirmation."""

    del replaced_existing
    return "\n".join(
        [
            f"Loaded `{workspace.original_file_name}`.",
            "",
            f"- {workspace.row_count} rows",
            f"- {workspace.column_count} columns",
        ]
    )


def _render_external_validation_workspace_success(
    workspace: TabularWorkspace,
) -> str:
    """Report an external validation candidate without replacing training state."""

    return "\n".join(
        [
            f"Loaded the separate validation dataset `{workspace.original_file_name}`.",
            "",
            f"- {workspace.row_count} rows",
            f"- {workspace.column_count} columns",
            "",
            "Your current training dataset remains unchanged.",
            "Which column contains the prediction outcome in this validation dataset?",
        ]
    )

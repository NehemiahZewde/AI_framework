        ...
        response = await agent.respond(
            ...
            runtime_context=MLAgentContext(
                ...
                ml_project_state=ml_project_states.get_or_create(
                    chainlit_session_id,
                    tabular_workspaces.get(chainlit_session_id),
                ),
            ),
        )
    except Exception as exc:
...
def _render_local_workspace_success(
    ...
    lines = [
        f"`{workspace.original_file_name}` is also prepared locally for controlled ML setup operations.",
    ...
    if replaced_existing:
        ...
        lines.append("- The previous local ML setup was reset.")
    return "\n".join(lines)
